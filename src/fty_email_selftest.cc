...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_EMAIL_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_email_private_selftest (verbose, item->subtest);
#endif // FTY_EMAIL_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        ...
        if (!test->subtest)
            ...
            fty_email_private_selftest (verbose, test->subtest);
#endif // FTY_EMAIL_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
