...
default|default-Werror|default-with-docs|valgrind|clang-format-check)
    ...
    ; then
        ...
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 -b release/IPM_Infra-1.5.0 https://github.com/42ity/fty-common-translation.git fty-common-translation
        cd ./fty-common-translation
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
        if [ -e autogen.sh ]; then
